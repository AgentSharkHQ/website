...
vec3 gradientHash(vec3 p) {
  ...
  return vec3(cos(phi + TAU * h.y), sin(phi + TAU * h.y) * cos(phi), 2.0 * h.z - 1.0);
}
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    // Skip WebGL shader on small screens — too heavy for mobile GPU.
  ...
  return (
    <>
      <div ref={containerRef} className="soft-aurora-container" />
      <div className="md:hidden absolute inset-0 pointer-events-none" aria-hidden="true" style={{
        background: 'radial-gradient(ellipse at 70% 30%, rgba(30, 91, 217, 0.18) 0%, transparent 60%)',
      }} />
    </>
  );
}
